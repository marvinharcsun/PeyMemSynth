...
package com.group3.synthesizerapp.knob;
...
import com.group3.synthesizerapp.R;
...
public class KnobView extends View {
  ...
  public KnobView(Context context, AttributeSet attrs) {
    ...
    density_ = getResources().getDisplayMetrics().density;
      sensitivity_ = (.005f)/density_; // TODO: should be configurable
    strokeWidth_ = 1.0f * density_;
    rect_ = new Rect();
    rectF_ = new RectF();
    innerRectF_ = new RectF();
    textHeight_ = 0f * density_;  // probably should be set by XML parameter
  ...
  protected void onDraw(Canvas canvas) {
    ...
    paint_.setColor(0xFFFF4081);
    paint_.setStyle(Style.STROKE);
    paint_.setStrokeWidth(rectF_.width() * 0.10f);
    ...
    float sweepAngle = (float)knobValue_ * range;
      ...
      float length = innerRectF_.width()*.5f;

      canvas.drawLine(innerRectF_.centerX(),innerRectF_.centerY(),
              innerRectF_.centerX()+ length*(float)Math.cos((sweepAngle+startAngle)*radians),
              innerRectF_.centerY()+ length*(float)Math.sin((sweepAngle+startAngle)*radians),paint_);
